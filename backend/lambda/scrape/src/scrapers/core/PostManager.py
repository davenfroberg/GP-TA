...
from config.constants import (
    ...
    MAJOR_UPDATE_TYPES,
    QUESTION_UPDATE_TYPES,
    S_ANSWER_UPDATE_TYPES,
    SES_RECIPIENT_EMAIL,
)
...
class PostManager:
        ...
        try:
            ...
            logger.exception(
                ...
                extra={
                    "course_id": course_id,
                    "post_id": post_id,
                    "had_major_update": had_major_update,
                },
            )
    ...
    def put_new_diffs(self, new_post, course_id, old_num_changes):
        ...
        for change in new_changes:
            ...
            had_major_update = (
                self.handle_individual_change(change, new_post, course_id, sequence)
                or had_major_update
            )
            sequence += 1
    ...
    def handle_new_post(self, post, course_id):
        ...
        if self._should_notify(post):
            notification_config = NotificationConfig(recipient_email=SES_RECIPIENT_EMAIL)
    ...
    def process_post(self, new_post, course_id):
        ...
        try:
            ...
            logger.exception(
                "Failed to fetch post from DynamoDB",
                extra={"course_id": course_id, "post_id": post_id},
            )
            raise
